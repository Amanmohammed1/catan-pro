  ...
  return (
    <div className="relative h-full w-full">
      ...
      {showStats && stats !== null && (
        <div className="pointer-events-none absolute bottom-3 left-3 rounded-md bg-surface-900/80 px-2.5 py-1.5 font-num text-[11px] text-ink-300 tabular-nums">
...
function CameraControls({
  ...
  return (
    <button
      type="button"
      onClick={onReset}
      className="absolute right-3 top-3 rounded-lg border border-surface-600 bg-surface-800/85 px-3 py-1.5 text-xs font-medium text-ink-300 shadow-lift backdrop-blur transition hover:bg-surface-700 hover:text-ink-100"
    >
      Reset view
    </button>
  );
